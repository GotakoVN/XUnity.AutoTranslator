...
using XUnity.Common.Logging.SimpleLogger.Logging;
using XUnity.Common.Logging.SimpleLogger.Logging.Handlers;
using XUnity.Common.Logging.SimpleLogger.Logging.Module;

namespace XUnity.Common.Logging.SimpleLogger
{
    public static class Logger
    {
        ...
        public static void DefaultInitialization()
        {
            ...
            Log( LogLevel.Info, "Default initialization");
        }
        ...
        private static void Log( LogLevel level, string message, string callingClass, string callingMethod, int lineNumber)
        {
        ...
        {
            public static bool ByLevelHigher( LogLevel logMessLevel, LogLevel filterLevel )
            {
                return ((int)logMessLevel >= (int)filterLevel);
            }

            public static bool ByLevelLower( LogLevel logMessLevel, LogLevel filterLevel )
            {
                return ((int)logMessLevel <= (int)filterLevel);
            }

            public static bool ByLevelExactly( LogLevel logMessLevel, LogLevel filterLevel )
            {
                return ((int)logMessLevel == (int)filterLevel);
            }
        ...
        public class FilterByLevel
        {
            ...
            public Predicate<LogMessage> Filter { get { return delegate(LogMessage logMessage) {
                return FilterPredicates.ByLevel(logMessage, FilteredLevel, delegate( LogLevel lm, LogLevel fl ) {
                return ExactlyLevel ? 
